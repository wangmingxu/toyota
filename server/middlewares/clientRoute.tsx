...
router.use(async (req, res) => {
  ...
  const currentRoute = matchRoutes(
    routes,
    originalUrl.replace(url.parse(originalUrl).search as string, ''),
  );
  // console.log(currentRoute);
  // 通过组件上的getInitialProps静态方法获取数据
  const promises = currentRoute
    .map(
      ({ route, match }) => ((route.component as any).getInitialProps
        ? (route.component as any).getInitialProps({ injector, match, query })
        : null),
    )
    .filter(item => !!item);
  try {
